...
``shapiq.vision`` ships four masking strategies, selected with the
``masking_strategy`` argument of the architecture.

Pixel-space strategies replace removed pixels and are used with
:class:`~shapiq.vision.CNNArchitecture`:
...
used with :class:`~shapiq.vision.TransformerArchitecture`:
...
Mixing domains -- a pixel strategy on ``TransformerArchitecture`` or a token
strategy on ``CNNArchitecture`` -- raises a ``TypeError`` at construction.
"""
...
# --------------------------
# Pass the strategy to :class:`~shapiq.vision.CNNArchitecture`. Different
...
from shapiq.vision import CNNArchitecture, ImageExplainer
...
# --------------------------
# Token strategies go to :class:`~shapiq.vision.TransformerArchitecture` the
...
from shapiq.vision import MaskTokenStrategy, TransformerArchitecture
...
# ``TransformerArchitecture(model=model, processor=processor,
