...
information from that region enters the transformer.
:class:`~shapiq.vision.ImageExplainer` picks this token-masking path
automatically when the model verifiably supports it (see
:ref:`sphx_glr_auto_examples_vision_plot_03_architectures.py`).
"""
...
# Explain
# -------
# Pass the raw model plus its processor; the explainer routes the model to
# :class:`~shapiq.vision.TransformerArchitecture` and masks in token space.
# The default splits ViT-base's 14x14 token grid into 4 quadrant players, so
# :math:`2^4 = 16` coalitions cover the whole game and ``budget=16`` is
# exact. For finer-grained players see
# :ref:`sphx_glr_auto_examples_vision_plot_04_player_strategies.py`.

from shapiq.vision import ImageExplainer

explainer = ImageExplainer(model=model, data=image, processor=processor, random_state=42)
