...
# Explain
# -------
# :class:`~shapiq.vision.ImageExplainer` does the rest: it segments the image
# into roughly 10 SLIC superpixels, replaces removed superpixels with the
# image's mean color, and estimates Shapley values from ``budget`` model
# evaluations. By default it explains the model's top predicted class. With
# 10 players there are :math:`2^{10}` possible coalitions; a budget at or
# above that evaluates all of them and makes the values exact.
#
# Under the hood the raw model is wrapped in a
# :class:`~shapiq.vision.CNNArchitecture` that bundles the player and
# masking strategies -- see
# :ref:`sphx_glr_auto_examples_vision_plot_03_architectures.py` for that
# hierarchy and how to construct it yourself.

from shapiq.vision import ImageExplainer

explainer = ImageExplainer(model=model, data=image, random_state=42)
print(f"{type(explainer.architecture).__name__} with {explainer.imputer.n_features} players")
